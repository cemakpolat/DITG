...
int main(int argc, char *argv[])
{
	...
	if (elab_type == 1) {
		...
		for (z = 0; z < flownum; z++) {
			...
			//add here json packet Mursel Cem
						printf("{");

						printf("\"Flow Result\":[{");
						printf("\"Flow Number\":\"%u\",",flowinfo[z].flowId);
						printf("\"FromIP\":\"%s\",", flowinfo[z].srcaddress);
						printf("\"FromPort\":\"%u\",", flowinfo[z].srcport);
						printf("\"ToIP\":\"%s\",",flowinfo[z].destaddress);
						printf("\"ToPort\":\"%u\",",flowinfo[z].destport);

						printf("\"Total Time\":\"%13.6lf\",",totaltime);
						printf("\"Total Packet\":\"%13lu \",",flowinfo[z].pktrecv);
						printf("\"Minimum Delay\":\"%13.6lf\",",flowinfo[z].mindelay);
						printf("\"Maximum Delay\":\"%13.6lf\",",flowinfo[z].maxdelay);
						printf("\"Average Delay\":\"%13.6lf\",",flowinfo[z].avdelay);
						printf("\"Average Jitter\":\"%13.6lf\",",flowinfo[z].avjitter);
						printf("\"DelayStd Deviation\":\"%13.6lf\",",flowinfo[z].delvariation);
						printf("\"BytesRX\":\"%13.0lf\",",byterecv);
						if(totaltime>0) {
							printf("\"Average Bitrate\":\"%13.6lf\",",flowinfo[z].byterecv*8/1000/totaltime);
							printf("\"Average Packet Rate\":\"%13.6lf\",",(double)flowinfo[z].pktrecv/totaltime);
						}
			  			else {
							printf("\"Average Bitrate\":\"%13.6lf\",",(double)0);
							printf("\"Average Packet Rate\":\"%13.6lf\",",(double)0);
						}
						if(flowinfo[z].pktrecv > 0 && flowinfo[z].pktloss > 0){
							printf("\"Packet Dropped\": \"%13lu (%3.2lf %%)\",", flowinfo[z].pktloss,(double) 100
								* flowinfo[z].pktloss / (flowinfo[z].pktrecv + flowinfo[z].pktloss));
							printf("\"Average loss-burst Size\":\"%13lf\"",(double)flowinfo[z].pktloss/flowinfo[z].lossEvents);
							}
						else {
							printf("\"Packet Dropped\": \"%13lu (%3.2lf %%)\",",(long unsigned int)0,(double)0);
							printf("\"Average Loss-Burst Size\": \"%13lf\"", (double)0);
						}

						if(z+1==flownum){//end of flow result json string
							printf("}");
						}
						else{
							printf("},");
...
/*			printf("----------------------------------------------------------\n");
		...
		if (pktrecv > 1)
			...
			avjitter = 0;
		// JSON print Mursel Cem

				printf("\"Total Result\":{");
				printf("\"Flow Number\":\"%13u\",",flownum);
				printf("\"Total Time\":\"%13.6lf\",",totaltime);
				printf("\"Total Packet\":\"%13.0lf\",",pktrecv);
				printf("\"Minimum Delay\":\"%13.6lf\",",mindelay);
				printf("\"Maximum Delay\":\"%13.6lf\",",maxdelay);
				printf("\"Average Delay\":\"%13.6lf\",",avdelay);
				printf("\"Average Jitter\":\"%13.6lf\",",avjitter);
				printf("\"DelayStd Deviation\":\"%13.6lf\",",delvariation);
				printf("\"BytesRX\":\"%13.0lf\",",byterecv);
				if(totaltime > 0){
					printf("\"Average Bitrate\":\"%13.6lf\",",byterecv*8/1000/totaltime);
					printf("\"Average Packet Rate\":\"%13.6lf\",",pktrecv/totaltime);
				}
				else {
					printf("\"Average Bitrate\":\"%13.6lf\",",(double)0);
					printf("\"Average Packet Rate\":\"%13.6lf\",",(double)0);
				...
				if(pktrecv > 0 && pktloss > 0){
					printf("\"Packet Dropped\":\"%13.0u (%3.2lf %%)\",", pktloss, (double)(100*pktloss/(pktrecv+pktloss)));
					printf("\"Average Loss-Burst Size\":\"%13lf\",",(double)pktloss/lossEvents);
				}
				else {
		    			printf("\"Packet Dropped\":\"%13.0lf (%3.2lf %%)\",",(double)0,(double)0);
					printf("\"Average Loss-Burst Size\":\"%13.0lf\",", (double)0);
				}
				printf("\"Error lines\": \"%13lu\"",pkterr);
				printf("}");
				printf("}");
		/*
