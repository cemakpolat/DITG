...
//#ifdef UNIX
	...
	//CEM: Possible option that works for this guy: http://igourd.blogspot.com/2009/05/work-around-on-pthreadcancel-for.html
	...
	if (idThread != 0){
		...
		if ( (ret = pthread_kill(idThread, SIGUSR1)) != 0)
		{
		    printf("Error cancelling thread %d, error = %d ", idThread, ret);
		}
	}
	if (ret != 0)
			ret = -1;
